<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8" import="java.util.ResourceBundle, java.util.Locale, com.epam.jwd.apotheca.controller.action.CartAction, java.util.Map, java.util.HashMap"%>
...
<%
	...
	int pagesCount = totalCount / pageSize + ((totalCount % pageSize) == 0 ? 0 : 1);
...
<c:if test="${(not empty action.cart) and (not empty action.products) }">
	...
	<div class="container">
		<div style="overflow: hidden" align="center">
			<div style="float: left">
				...
				<%=currentPage * pageSize - pageSize + 1 + ((totalCount % pageSize != 0
			...
			<div style="float: right">
				<div style="float: left">
					...
					<select name="pageSize" onChange="displayCart( 1, this.options[this.selectedIndex].value );">
	...
	<table border = "1" style="margin-top: 20px" class="container">
				...
				<c:when test="${not empty action.cart}">
					...
					<c:forEach items="${action.products}" var="d">
						...
						>
							...
							>${d.key.price }</td>
							<td>
								<input type="number" min="1" max="100" id="cartAmount${d.key.id}"
	...
	<form action="drugsBill.run" method="POST">
		<div id = "div" <c:if test="${empty action.cart.products}">style="display:none"</c:if> class="container" align="center">
			<div id="totalAmount1">
				<font color="Blue"><%= rb.getString("drugs.total") %></font>
				<c:out value="${totalA }"/>
				<input id="buyButton" type="submit" class="btn btn-primary" <c:if test="${action.cart.invalid }">disabled</c:if> value="<%=rb.getString("drugs.buy")%>"/>
			</div>
		</div>
	</form>

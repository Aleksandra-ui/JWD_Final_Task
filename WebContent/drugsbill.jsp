				...
				<c:when test="${not empty drugsList}">
					<c:forEach items="${drugsList}" var="d">
						...
						<tr
							...
							<td><input type="number" value=0 disabled id="amount${d.id}"
								onchange="printAmount(${d.id });" onkeyup="saveAmount(this);" /></td>
							<td><c:if test="${not empty drugsFromRecipe[d.id] }">
									<c:out value="${drugsFromRecipe[d.id] }" />
								</c:if></td>
							<td><c:set var="present" value="false" /> <c:set var="ids"
									value="${fn:split(param.drugIds,',')}" /> <c:set var="idStr">${d.id}</c:set>
								<c:forEach var="id" items="${ids}">
									<c:if test="${id == idStr}">
										<c:set var="present" value="true" />
									</c:if>
								</c:forEach> <input type="checkbox" id="drug${d.id}" value="${d.id}"
								name="drug"
								onchange="addRemoveFromCart(this, ${d.id});showId(this);"
								<c:out value="${present ? 'checked' : ''}"/> /> <input
								type="hidden" id="checkbox${d.id}"
								value="${d.name}&nbsp;|&nbsp;${d.dose}&nbsp;|&nbsp;${d.price}" />
							</td>
						</tr>
